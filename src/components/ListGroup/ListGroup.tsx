...
const ListGroup: React.FC<Props> = ({ image, name, title, className, id }) => {
  ...
  return (
    <div className={"flex items-center px-6 py-3 m-4 border-2 rounded-xl shadow-lg" + className}>
      <img src={image} alt="" className="h-10 w-10" />
      <div className="flex flex-col pl-6 ">
        <Link className = "hover:border-b-2 hover:border-black w-max " to = {link}><h1>{name}</h1></Link>
        <h3>{title}</h3>
      </div>
    </div>
  );
